...
public:
  ...
  /**
   * Allocate memory for coils, discrete inputs, holding registers, or input register values.
   *
   * @param type type of read to perform, either COILS, DISCRETE_INPUTS,
   *             HOLD_REGISTERS, or INPUT_REGISTERS
   * @param nb number of values to allocate
   *
   * @return 0 on failure, 1 on success
   **/
  int AllocateMemory(int type, int nb);
