...
int TrueRandomClass::randomBit(void) {
  ...
  //
  //int a = randomBitRaw2() | (randomBitRaw2()<<1);
  int a = analogRead(14) | (analogRead(15) << 1);
...
uint8_t TrueRandomClass::randomByte(void) {
  uint8_t result;
  uint8_t i;
  result = 0;
  ...
  {
    result |= (randomBit() << i);
    //Serial.print("i:"); Serial.print(i); Serial.print(" result:"); Serial.println(result);
  }
  return result;
}

uint16_t TrueRandomClass::rand()
{
  uint16_t result;
  uint8_t i;
  result = 0;
...
uint32_t TrueRandomClass::random()
{
  uint32_t result;
  uint8_t i;
  result = 0;
